...
module DirectionsOfMovementRobot # "робот на клетчатом поле со сторонами горизонта"

export DirectionsOfMovement, Up, Down, Left, Right, Robot, move!, isborder, putmarker!, ismarker, temperature, show, show!, save, sitedit, sitcreate

"""
    @enum DirectionsOfMovement Up=0 Left=1 Down=2 Right=3

-- Определяет стороны горизонта на клетчатом поле с роботом: Up - Север (вверху), Left - Запад (слева), Down - Юг (внизу), Right - Восток (справа)     
"""
@enum DirectionsOfMovement Up=0 Left=1 Down=2 Right=3
...
module SituationData
    using PyPlot, ...DirectionsOfMovementRobot #: DirectionsOfMovement
    ...
    mutable struct Situation
        ...
        borders_map::Matrix{Set{DirectionsOfMovement}} # - матрица множеств запрещенных направлений: каждый ее элемент соответствует некоторой клетке поля (только в его видимой части) и содержит множество направлениями (::DirectionsOfMovement), в которых имеются перегородки (внешняя рамка, даже если она есть, здесь в расчет не берется)
    ...
    function init_default(frame_size::Tuple{Integer,Integer})
        #!изменения!
        #случайная позиция для робота
        robot_position = (rand(1:frame_size[1]), rand(1:frame_size[2]))
        #robot_position = (frame_size[1], 1) # по умолчанию начальное положение - левый нижний угол (Down-Left)
    ...
    function load(file_name::AbstractString) 
        ...
        for k ∈ eachindex(borders_map) 
            line = strip(readline(io))
            isempty(line) || (borders_map[k] = Set(DirectionsOfMovement.(parse.(Int, split(line)))))
        end
    ...
    function adjacent_position(position::Tuple{Integer,Integer},side::DirectionsOfMovement)
    ...
    function is_inner_border(position::Tuple{Integer,Integer}, side::DirectionsOfMovement, borders_map::Matrix{Set{DirectionsOfMovement}})
    ...
    # (в матрице borders_map каждому элементу соответствует "актуальная" позиция: из 2х соседних позиций только одна может быть "актуальной")
        if side ∈ borders_map[position...]
            ...
            side = DirectionsOfMovement(mod(Int(side)+2, 4)) # - противоположное направление, относительно первоначального
    ...
    function handle_button_press_event!(event, file::AbstractString)
        ...
        function set_or_del_border!(position,side::DirectionsOfMovement) 
        ...
        elseif Δy >= abs(Δx) && Δy >= ρ   
            set_or_del_border!(position,Up)
        elseif Δx <= -abs(Δy) && Δx <= -ρ 
            set_or_del_border!(position,Left)
        elseif Δy <= -abs(Δx) && Δy <= -ρ 
            set_or_del_border!(position,Down)
        elseif Δx >= abs(Δy) && Δx >= ρ   
            set_or_del_border!(position,Right)
...
-- Конструктор объектов типа Robot (исполнитель "Робот на клетчатом поле со сторонами горизонта типа DirectionsOfMovement"):
...
Командный интерфейс исполнителя (объекта) типа Robot: `mowe`, `isborder`, `putmarker`, `ismarker`, `temperature`, `show`, `show!` (см. help)
...
"""
    move!(r::Robot, side::DirectionsOfMovement)::Nothing
...
"""
function move!(r::Robot, side::DirectionsOfMovementRobot.DirectionsOfMovement)
...
"""
    isborder(r::Robot, side::DirectionsOfMovement)::Bool

-- Проверяет наличие перегородки в заданном направлении
"""
function isborder(r::Robot,side::DirectionsOfMovementRobot.DirectionsOfMovement)::Bool
    i,j = position(r)
    ...
    elseif 1<=i<=imax && j<1 # - (i,j) - за пределами фрейма (в западном секторе)
        ...
        if r.situation.is_framed == true
            if side==Up && i==1 || side==Down && i==imax || side==Left && j==1 || side==Right && j==jmax
